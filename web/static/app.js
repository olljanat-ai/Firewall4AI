...
function editDiskImage(id) {
  ...
  if (aiTools.includes('opencode')) document.getElementById('disk-image-ai-opencode').checked = true;
...
async function submitDiskImage() {
  ...
  const ai_tools = [];
  const aiValue = document.querySelector('input[name="disk-image-ai-tool"]:checked')?.value || 'none';
  if (aiValue !== 'none') ai_tools.push(aiValue);
...
async function loadAgentVMs() {
  try {
    ...
    currentAgents.forEach(a => {
      ...
      if (sshTarget) {
        const vscodeURI = 'vscode://vscode-remote/ssh-remote+root@' + encodeURIComponent(sshTarget) + '/root?windowId=_blank';
